...
\begin{abstract}
Agile and Scrum teams often face inconsistent task assignment, uncertain deadline estimation, and high manual overhead during sprint execution. This study investigates the feasibility of embedding machine learning predictions into an automated Agile workflow. Using a subset of the Aurora project data (N=824), an XGBoost-based multi-class classifier is trained for assignee recommendation, achieving \textbf{77.58\% Top-1 accuracy} and \textbf{95.15\% Top-3 accuracy} on an unseen hold-out set. For deadline prediction, the study adopts a probabilistic approach using a combined dataset (N=1,171). A Random Forest model classifies issues into urgency buckets with \textbf{41.28\% accuracy} (outperforming a 24\% baseline) and estimates 90\% confidence intervals via quantile regression. The interval model achieves \textbf{88.09\% coverage} with a \textbf{Median MAE of 6.16 days}, showing a statistically significant improvement ($p < 0.05$) over global median baselines. The models are deployed as REST endpoints and integrated into an n8n-orchestrated workflow, enabling automated field updates and periodic re-evaluation during active sprints.
\end{abstract}
...
\centering
    \centering
    \includegraphics[width=0.8\linewidth]{figures/system_flow.png}
...
\begin{itemize}
    ...
    \item \textbf{Deadline Dataset:} To build a more robust time estimator, \textit{Aurora} and \textit{Mesos} data were combined (initial N=1,171). Records were filtered to the 0.1--30 day resolution range to align with standard Scrum sprint cycles, and issues with 0 story points were removed. This yielded 1,171 high-quality issue records for training.
\end{itemize}
...
Assignee recommendation is formulated as a multi-class classification problem with $K=8$ classes. XGBoost was employed to learn an additive ensemble of trees. The predicted probability for class $i$ is computed using the softmax function over the sum of trees in XGBoost, as shown in Equation~\ref{eq:xgb_pred}.
\begin{equation}
\hat{\mathbf{p}}_i = \mathrm{softmax}\Big(\sum_{t=1}^{T} f_t(\mathbf{x}_i)\Big),
\label{eq:xgb_pred}
\end{equation}
where $f_t(\mathbf{x}_i)$ represents the output of the $t$-th tree in the ensemble for the input feature vector $\mathbf{x}_i$.
Model hyperparameters were selected using grid-based cross-validation on the training data, resulting in a learning rate of 0.05, a maximum tree depth of 10, and 400 estimators.

% ---------------------------------------------------------
\subsection{Deadline Prediction Model}
% ---------------------------------------------------------

Given the high variance in agile issue resolution times, deadline estimation is modeled using a dual approach to maximize reliability:
\begin{enumerate}
    \item \textbf{Urgency Classification:} A Random Forest Classifier assigns issues to one of four urgency buckets: \textit{Quick} ($\le$3d), \textit{Medium} (4-7d), \textit{Extended} (8-14d), and \textit{Long} ($>$14d).
    \item \textbf{Interval Prediction:} A Quantile Regressor estimates the 90\% confidence interval ($[q_{0.05}, q_{0.95}]$) for resolution time. This provides stakeholders with a probabilistic range rather than a brittle point estimate.
\end{enumerate}

Random Forest was selected for both tasks due to its robustness to noise. The final model utilizes \texttt{n\_estimators=300} and \texttt{max\_depth=25}.
...
To assess model performance objectively, the following metrics were employed:

\subsubsection{Assignee Recommendation (Classification)}
\begin{itemize}
    \item \textbf{Top-1 Accuracy:} Shows how often the model’s most confident prediction matches the actual assignee in the test data.
    \item \textbf{Top-3 Accuracy:} Indicates the percentage of cases where the correct assignee appears among the top three recommendations.
    \item \textbf{F1-Score (Macro):} Combines precision and recall into a single score and averages the result across all classes, so that each assignee is treated equally despite data imbalance.
\end{itemize}

\subsubsection{Deadline Prediction (Regression \& Interval)}
\begin{itemize}
    \item \textbf{Classification Accuracy:} Used for the 4-class urgency buckets (\textit{Quick} to \textit{Long}).
    \item \textbf{Interval Coverage (PICP):} The percentage of true resolution times that fall within the predicted 90\% confidence interval ($[q_{0.05}, q_{0.95}]$). Ideally, this should approach 90\%.
    \item \textbf{Median Absolute Error (MedAE):} Represents the median difference between the predicted completion time and the real resolution time. This metric is preferred over MAE because issue durations are not evenly distributed.
\end{itemize}
...
Experiments were executed on a local workstation (AMD Ryzen 9, NVIDIA RTX 3060). For assignee recommendation, evaluation was performed on a stratified hold-out set of 165 issues (20\% of 824). For deadline prediction, evaluation utilized a random 20\% test split of the 1,171 filtered issues (N=235 test samples).
...
A detailed per-class analysis (Table \ref{tab:assignee_class_report}) highlights the model's robustness across different developer profiles. High-volume contributors like Maxim K. (Support=53) maintain solid performance (F1=0.79), while specific experts like Brian W. and Joshua C. achieve F1-scores $\ge$ 0.90. The model struggles most with rare classes, such as David M. (Support=5), where it achieves perfect precision (1.00) but low recall (0.20), indicating a conservative prediction strategy for developers with fewer historical labels.

\begin{table}[t]
\caption{Per-Class Performance on Unseen Test Set (Assignee Model)}
\centering
\renewcommand{\arraystretch}{1.1}
\begin{tabular}{l c c c r}
\hline
\textbf{Developer} & \textbf{Precision} & \textbf{Recall} & \textbf{F1-Score} & \textbf{Support} \\
\hline
David M.   & 1.00 & 0.20 & 0.33 & 5 \\
Joshua C.  & 0.87 & 0.93 & 0.90 & 14 \\
Kevin S.   & 0.95 & 0.83 & 0.88 & 23 \\
Maxim K.   & 0.77 & 0.81 & 0.79 & 53 \\
Mark C.    & 0.80 & 0.75 & 0.77 & 16 \\
Bill F.    & 0.62 & 0.72 & 0.67 & 32 \\
Brian W.   & 0.91 & 0.91 & 0.91 & 11 \\
Zameer M.  & 0.70 & 0.64 & 0.67 & 11 \\
\hline
\end{tabular}
\label{tab:assignee_class_report}
\end{table}
...
Due to the stochastic nature of issue resolution, this study evaluated the model's ability to categorize urgency and estimate reliable time intervals, rather than relying solely on point error.

\subsubsection{Urgency Classification}
Table~\ref{tab:deadline_class} shows the classification performance into four urgency buckets. The Random Forest model achieved \textbf{41.28\%} accuracy. While modest in absolute terms, this strictly outperforms the random baseline (24.26\%) and XGBoost/SVM alternatives. Precision was highest for the extreme classes (\textit{Quick} and \textit{Long}), indicating the model effectively identifies trivial tasks and major blockers.

\begin{table}[t]
\centering
\caption{Deadline Urgency Classification (4-Class).}
\renewcommand{\arraystretch}{1.08}
\begin{tabular}{l c}
\hline
\textbf{Model} & \textbf{Accuracy} \\
\hline
Dummy (Baseline) & 24.26\% \\
SVM (Linear) & 31.91\% \\
XGBoost & 36.60\% \\
\textbf{Random Forest (Tuned)} & \textbf{41.28\%} \\
\hline
\end{tabular}
\label{tab:deadline_class}
\end{table}

\subsubsection{Interval Reliability}
The interval prediction model (90\% CI) demonstrated high reliability:
\begin{itemize}
    \item \textbf{Coverage:} \textbf{88.09\%} of true resolution times fell within the predicted intervals, very close to the 90\% target.
    \item \textbf{Median MAE:} \textbf{6.16 days}, compared to a global median baseline of 6.83 days.
    \item \textbf{Significance:} A Wilcoxon Signed-Rank Test between the model errors and baseline errors yielded a p-value of \textbf{0.001657} ($<0.05$), confirming the model provides a statistically significant improvement over naive guessing.
\end{itemize}
...
The integration demonstrated operational feasibility. The ``bi-daily'' (every 12 hours) re-evaluation loop successfully identified issues with missing fields and corrected them without user intervention. By providing an urgency category and a confidence interval, the system allows project managers to focus on outliers (e.g., predicted "Long" tasks) rather than micromanaging every due date.

% =========================================================
\section{Conclusion}
% =========================================================

This work demonstrates the feasibility of embedding ML inference into an automated Agile workflow. The assignee model achieves 77.58\% Top-1 and 95.15\% Top-3 accuracy, proving effective for recommendation-based assignment. For deadline prediction, the study highlights the "accuracy ceiling" in noisy agile data but establishes a reliable alternative: an interval prediction model with 88.09\% coverage and a statistically significant reduction in median error (6.16 days) compared to baselines. These results confirm that while precise deterministic forecasting remains challenging in agile environments, probabilistic modeling coupled with automated workflow integration offers a scientifically valid and operationally robust solution for reducing manual coordination overhead.

\textbf{Limitations of this Study:} Despite the promising results, several limitations exist. First, the evaluation relies on open-source datasets (Aurora and Mesos), which may not fully represent the proprietary workflows or diverse development cultures found in closed-source enterprise environments. Second, the assignee recommendation model assumes a fixed set of developers ($K=8$); in practice, agile teams experience turnover, necessitating a dynamic retraining pipeline to handle new or departing members. Finally, the deadline model's accuracy is heavy dependent on the quality of textual metadata (summary and description); issues with sparse or ambiguous descriptions may lead to wider confidence intervals or misclassification.

The primary contribution is the workflow-aware operationalization: REST-deployed predictors embedded into n8n orchestration. Future work will focus on three key areas. First, we aim to implement a "Human-in-the-Loop" mechanism where developer feedback on assignments is used for online model retraining, allowing the system to adapt to evolving team dynamics. Second, we plan to integrate Large Language Models (LLMs) to extract deeper semantic context from issue discussions and attachments, potentially improving urgency classification accuracy. Finally, we intend to incorporate additional process signals (e.g., PR activity) to further narrow prediction intervals and conduct a longitudinal A/B test in a live software development environment to quantitatively measure the impact on sprint velocity.
